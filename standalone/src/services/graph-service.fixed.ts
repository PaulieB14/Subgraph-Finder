...
export async function matchIntent(intent: string, network?: string): Promise<MatchResponse> {
  ...
  try {
    ...
    // If no subgraphs match, return empty result
    ...
    try {
      // Use Gemini to match the intent with available subgraphs
      const model = genAI.getGenerativeModel({ model: "gemini-2.5-pro-exp-03-25" });
      
      // Create a prompt for Gemini
      const prompt = `
        I need to find the most relevant subgraph(s) for this user intent:
        "${intent}"
        
        Available subgraphs:
        ${filteredSubgraphs.map(sg => `- ${sg.displayName} (${sg.network}): ${sg.description}`).join('\n')}
        ...
        { "id": "subgraph-id", "score": number, "reason": "explanation" }
      `;
      
      console.log("Sending intent matching request to Gemini");
      const result = await model.generateContent(prompt);
      const response = await result.response;
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          matches = parsed.map(item => {
            ...
            const subgraph = filteredSubgraphs.find(sg => sg.id === item.id) || 
                            filteredSubgraphs.find(sg => sg.displayName === item.id);
            
            // Generate a sample GraphQL query based on the subgraph
            const sampleQuery = generateSampleQuery(subgraph || filteredSubgraphs[0]);
            
            return {
              subgraph: subgraph || filteredSubgraphs[0],
        ...
        // Fall back to a simple match
        matches = filteredSubgraphs.map(sg => ({
          subgraph: sg,
          confidence: 50,
      ...
      if (matches.length > 0 && matches[0].confidence > 70) {
        recommendation = `The "${matches[0].subgraph.displayName}" subgraph is a good match for your needs.`;
      } else if (matches.length > 0) {
        recommendation = `There are some potential matches, but none are perfect. The best option might be "${matches[0].subgraph.displayName}".`;
      } else {
        recommendation = "No matching subgraphs found. You may need to create a new subgraph for your specific needs.";
      }
      ...
      console.error("Error matching intent with Gemini:", error);
      // Return a basic match without AI
      return {
        matches: filteredSubgraphs.map(sg => ({
          subgraph: sg,
          confidence: 50,
        ...
        recommendation: "AI matching unavailable. Here are some potential subgraphs that might help."
      };
    }
  } catch (error) {
...
/**
 * Generate a sample GraphQL query for a subgraph
 */
function generateSampleQuery(subgraph: Subgraph): string {
  // Default query for Uniswap V3
  ...
  // Generic query for other subgraphs
  return `{
